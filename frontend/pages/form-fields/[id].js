...
export default function EditFormField() {
  ...
  const fetchFormField = async () => {
    try {
      ...
      // Get country and visa type from form section
      const country = formField.formSection?.countryVisaType?.country?._id;
      const countryVisaType = formField.formSection?.countryVisaType?._id;
      ...
      setFormData({
        ...
        country: country || '',
        countryVisaType: countryVisaType || '',
        formSection: formField.formSection?._id || '',
        isActive: formField.isActive !== undefined ? formField.isActive : true,
