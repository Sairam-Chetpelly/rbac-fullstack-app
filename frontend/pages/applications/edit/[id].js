  ...
  const renderFilePreview = (fileData) => {
    ...
    if (isImage) {
      return (
        <div className="mt-4">
          <img 
            src={fileUrl || `${process.env.NEXT_PUBLIC_API_BASE_URL?.replace('/api', '') || 'https://api.oneworldvisa.in'}/uploads/applications/${filePath}`}
            alt={fileName}
                ...
                <div className="flex items-center gap-4">
                  <div className="w-100 h-100  rounded-2xl flex items-center justify-center shadow-lg">
                    {application.countryVisaType?.country?.placeImage ? (
                      <img 
                        src={`${process.env.NEXT_PUBLIC_API_BASE_URL?.replace('/api', '') || 'https://api.oneworldvisa.in'}/uploads/countries/${application.countryVisaType.country.placeImage}`} 
